// $Id: Skycal.cc 5029 2013-07-07 21:29:04Z flaterco $
...
const static double precisionJD = 15./86400.; // 15 seconds
...
// Added 2003-02-04 from Skycal 5 for moonrise/moonset
static void eclrot (double jd, double *x, double *y, double *z)
...
find_next_moon_phase (double &jd, int &phase) {
    ...
    while (newjd <= jd) {
        ...
        flmoon(++nlast,0,&newjd);
        kount++;
        if (kount > 5) { // Original limit was 35 (!)
            phase = -1; // was assert
            return;
        }
    }
...
static Tide::TideEvent findNextMoonPhase(const Tide::Timestamp& t) {

    Tide::TideEvent ev;
    int phase;
    double jd (t.jd());
    find_next_moon_phase(jd, phase);
    ev.time = Tide::Timestamp::fromJulianDate(jd);

    switch (phase) {
    case 0:
        ev.type = Tide::TideEvent::newmoon;
        break;
    case 1:
        ev.type = Tide::TideEvent::firstquartermoon;
        break;
    case 2:
        ev.type = Tide::TideEvent::fullmoon;
        break;
    case 3:
        ev.type = Tide::TideEvent::lastquartermoon;
        break;
    default:
        ev.type = Tide::TideEvent::invalid;
    }
...
static void find_next_rise_or_set (double &jd, double lat, double longit, bool lunar, bool &is_rise) {
...
static Tide::TideEvent findNextRiseOrSet (const Tide::Timestamp& t, const Tide::Coordinates &c, RiseSetType riseSetType) {
    Tide::TideEvent ev;
    bool isRise;
    double jd = t.jd();
    // skycal "longit" is measured in HOURS WEST, not degrees east.
    // (lat is unchanged)
    find_next_rise_or_set(jd, c.lat(), -(c.lng())/15.0, (riseSetType==lunar), isRise);
    ev.time = Tide::Timestamp::fromJulianDate(jd);
    if (isRise)
        ev.type = ((riseSetType == lunar) ? Tide::TideEvent::moonrise : Tide::TideEvent::sunrise);
    else
        ev.type = ((riseSetType == lunar) ? Tide::TideEvent::moonset : Tide::TideEvent::sunset);

    return ev;
}

void Skycal::AddSunMoonEvents(const Tide::Timestamp& start,
